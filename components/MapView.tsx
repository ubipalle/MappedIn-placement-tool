...
export default function MapView({
  ...
  // Load Mounts
  useEffect(() => {
    setMounts((mountsConfig as MountConfig).mounts);
    if ((mountsConfig as MountConfig).mounts.length > 0) {
      setSelectedMount((mountsConfig as MountConfig).mounts[0].id);
    }
  }, []);
  ...
  // 1. Placement Handler (Callback for MapContent)
  const handlePlaceCamera = useCallback((coord: { latitude: number, longitude: number }) => {
    ...
    const newCamera: Camera = {
        ...
        fieldOfView: mount.parameters.fieldOfView || defaultFOV,
        ...
        name: `${mount.name} ${cameras.length + 1}`,
  ...
  }, [selectedMount, mounts, cameras, defaultFOV, defaultRange, defaultHeight, defaultTilt, currentMapView]);


  // 2. Mouse Down (Start Drag) - Uses MAP COORDINATES (Reliable)
            ...
            if (wallSnap) {
                ...
                setCameras(prev => prev.map(c => {
                    if (c.id === dragCameraIdRef.current) {
                        return { ...c, latitude: dragGhost.lat, longitude: dragGhost.lng, rotation: finalRotation };
                    }
                    return c;
                }));
            }
  ...
  const handleUpdateCamera = (id: string, updates: Partial<Camera>) => setCameras(prev => prev.map(c => c.id === id ? { ...c, ...updates } : c));
       ...
       >
          ...
          >
             <div className="space-y-1">
               <label className="block text-xs font-medium text-gray-700">Camera Mount Type</label>
             ...
             <button onClick={handleExport} disabled={cameras.length === 0} className="w-full px-4 py-2 bg-green-600 text-white rounded font-medium disabled:bg-gray-300">Export</button>
          </div>
       ...
       <div className="w-80 bg-gray-50 border-l border-gray-200 overflow-y-auto">
         <div className="p-4">
           ...
           <div className="space-y-3">
             {cameras.map(c => (
               <div key={c.id} onClick={() => setSelectedCamera(c.id)} className={`p-3 rounded-lg cursor-pointer ${c.id === selectedCamera ? 'bg-blue-100 border-2 border-blue-500' : 'bg-white'}`}>
           ...
           {selectedCameraData && (
             <div className="mt-6 p-4 bg-white rounded-lg border border-gray-200 shadow-sm">
                ...
                <div className="space-y-4">
                   <div><label className="block text-sm font-medium">Name</label><input type="text" value={selectedCameraData.name} onChange={e => handleUpdateCamera(selectedCameraData.id, { name: e.target.value })} className="w-full border rounded px-2 py-1" /></div>
                   <div><label className="block text-sm font-medium">Height</label><input type="range" min="0.5" max="10" step="0.1" value={selectedCameraData.height} onChange={e => handleUpdateCamera(selectedCameraData.id, { height: parseFloat(e.target.value) })} className="w-full" /></div>
                   <div><label className="block text-sm font-medium">Tilt</label><input type="range" min="-90" max="30" value={selectedCameraData.tilt} onChange={e => handleUpdateCamera(selectedCameraData.id, { tilt: parseInt(e.target.value) })} className="w-full" /></div>
                   <div><label className="block text-sm font-medium">FOV</label><input type="range" min="30" max="180" value={selectedCameraData.fieldOfView} onChange={e => handleUpdateCamera(selectedCameraData.id, { fieldOfView: parseInt(e.target.value) })} className="w-full" /></div>
                   <div><label className="block text-sm font-medium">Range</label><input type="range" min="5" max="50" value={selectedCameraData.range} onChange={e => handleUpdateCamera(selectedCameraData.id, { range: parseInt(e.target.value) })} className="w-full" /></div>
                   <div><label className="block text-sm font-medium">Rotation</label><input type="range" min="0" max="360" value={selectedCameraData.rotation} onChange={e => handleUpdateCamera(selectedCameraData.id, { rotation: parseInt(e.target.value) })} className="w-full" /></div>
